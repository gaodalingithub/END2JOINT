...
import sys
import threading
# import utils  # not available in this environment; FK core matches original
...
class Arm_IK:
    def __init__(self, urdf_path, smooth=False):
        ...
        # Load model without geometry (meshes not available locally)
        self.robot = pin.RobotWrapper()
        self.robot.model = pin.buildModelFromUrdf(urdf_path)
        self.robot.data = self.robot.model.createData()
        self.robot.collision_model = pin.GeometryModel()
        self.robot.visual_model = pin.GeometryModel()
        self.robot.collision_data = None
        self.robot.visual_data = None
        # self.robot = pin.RobotWrapper.BuildFromURDF(urdf_path)
        self.joints_to_lock = ["left_gripper_a_joint", "left_gripper_b_joint", "right_gripper_a_joint", "right_gripper_b_joint"]
        ...
        self.reduced_robot.model.addFrame(
            pin.Frame('ee_right',
                      self.reduced_robot.model.getJointId('right_wrist_pitch_joint7'),
                      pin.SE3(
                          pin.Quaternion(1, 0, 0, 0),
                          np.array([0.15, 0.02, 0.0]),
                      ),
...
def test():
    ...
    urdf_path = os.path.join(current_dir, 'actibot_sdk/robot_description/v3/urdf/v3_urdf_251121-2.urdf')
    ik = Arm_IK(urdf_path)
    # test fk
    q = np.zeros(ik.reduced_robot.model.nq)
